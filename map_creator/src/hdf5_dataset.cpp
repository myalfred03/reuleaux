...
Hdf5Dataset::Hdf5Dataset(std::string fullpath)
{
  ...
  if (!seglist.empty())
  {
    std::copy(seglist.begin(), seglist.end() - 1, std::ostream_iterator<std::string>(oss_path, "/"));
...
{
  std::string fullpath = this->path_ + this->filename_;
  ROS_INFO("Opening Capability map %s", fullpath.c_str());
  this->file_ = H5Fopen(fullpath.c_str(), H5F_ACC_RDONLY, H5P_DEFAULT);
...
bool Hdf5Dataset::saveCapMapsToDataset(VectorOfVectors& capability_data, float& resolution)
{
  ...
  std::string fullpath = this->path_ + this->filename_;
  ROS_INFO("Saving Capability map %s", fullpath.c_str());
  ...
  this->capability_dataset_ = H5Dcreate2(this->group_capability_, "capability_dataset", H5T_NATIVE_DOUBLE,
                                         capability_dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

  H5Dwrite(this->capability_dataset_, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, dset2_data);
...
bool Hdf5Dataset::saveReachMapsToDataset(MultiMapPtr& poses, MapVecDoublePtr& spheres, float resolution)
{
  ...
  // Create the datset
  this->poses_dataset_ =
      H5Dcreate(this->group_poses_, "poses_dataset", H5T_NATIVE_FLOAT, file_space, H5P_DEFAULT, plist, H5P_DEFAULT);
  // Closing resources
  H5Pclose(plist);
  H5Sclose(file_space);
  ...
  this->sphere_dataset_ = H5Dcreate2(this->group_spheres_, "sphere_dataset", H5T_NATIVE_DOUBLE, sphere_dataspace,
  ...
  this->attr_ =
      H5Acreate2(this->sphere_dataset_, "Resolution", H5T_NATIVE_FLOAT, sphere_dataspace, H5P_DEFAULT, H5P_DEFAULT);
  H5Awrite(this->attr_, H5T_NATIVE_FLOAT, attr_data);
  // H5Aclose(this->attr_);
...
bool Hdf5Dataset::h5ToMultiMapPosesAndSpheres(MultiMapPtr& pose_col, MapVecDoublePtr& sphere_col)
{
  ...
  // As the data are read from the h5 file as chuck of array, the retrieved sphere addresses comes different for every
  // sphere But that is not the case for our map structure. Every sphere with same coordinate has same address in h5 So
  // to create the exact same Multimap and Map while maintaining the same strcture, we have to assign the spheres that
  // have the same coordinates, with the same address Otherwise when we will be loading the dataset for visualization or
  // other taks such as inverse map, the comparison between the coordinates of sphere dataset and coordinates of spheres
  // in Poses dataset are going to fail.
